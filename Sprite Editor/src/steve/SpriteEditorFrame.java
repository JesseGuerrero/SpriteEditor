...
public class SpriteEditorFrame extends javax.swing.JFrame {
	...
	private void addSpriteButtonActionPerformed(java.awt.event.ActionEvent evt) { //addsprite
		File dumpFolder = new File("D:\\Games\\Tools\\Sprite Editor\\Sprite Editor\\667_Sprite_Dump\\");
...
//		List<File> listOfSprites = orderedDump.subList(7000, orderedDump.size());
		...
		for(File sprite : listOfSprites) {
			totalAdded += 1;
			int id = Integer.parseInt(sprite.getName().split("_")[0]);
			if(brokenSpriteIds.contains(id))
				continue;
			Sprite newSprite = Sprite.get(cache, id);
			try {
				if(totalAdded % 200 == 0)
					System.out.println(sprite.getName() + ": " + totalAdded + " / " + listOfSprites.size());
				List<File> subSprites = new ArrayList<>();
				for(int subId = 0; subId < newSprite.images.size(); subId++) {
					String subSpriteName = sprite.getName().split("_")[0] + "_" + subId + ".png";
					File subSprite = new File(dumpFolder.getAbsolutePath() + "\\" + subSpriteName);
					if(subSprite.exists()) {
						if(subId > 1)
							System.out.println(subSprite.getAbsolutePath());
					} else {
						brokenSpriteIds.add(id);
						throw new FileNotFoundException();
					}
					subSprites.add(subSprite);
				}


				int subId = 0;
				for(File subSprite : subSprites) {
					BufferedImage image = ImageIO.read(subSprite);
